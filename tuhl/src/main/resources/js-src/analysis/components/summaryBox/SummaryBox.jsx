...
const SummaryBox = (props) => {
  ...
  const Text = (
    <TextPreview mrws={analysis?.mrws} text={analysis?.getText ? analysis.getText() : 'no text preview available'} />
  );
  ...
  const Comments = (
    <Comment text={analysis?.getComment && analysis.getComment() ? analysis.getComment() : 'no comments'} />
  );

  const Mappings = analysis?.getMappingTables().map((table, tableIdx) => {
    return (
      <MappingTable
        data={analysis}
        setData={() => {}}
