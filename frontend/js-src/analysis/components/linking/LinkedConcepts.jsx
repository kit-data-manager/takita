...
export const LinkedConcepts = ({ concepts, onDelete }) => {
  ...
  const rows = _concepts.map((c) => {
    return (
      <S.LinkedConceptItem key={c.uri}>
        <span>
          <S.ConceptA href={c.uri} target='_blank' rel='noreferrer' title='Open Concept in Skosmos'>
            {c.prefLabel}
          </S.ConceptA>
          &nbsp;
          <i>({c.parentLabel})</i>
  ...
  return <S.LinkedConcepts className='LinkedConcepts'>{rows}</S.LinkedConcepts>;
};
